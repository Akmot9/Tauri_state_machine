...
impl MachineState {
    ...
    pub fn init_machine(&self, window: tauri::Window, target: u32) {
        ...
        thread::spawn(move || {
            for count in 1..=target {
                ...
                loop {
                    ...
                    println!(
                        "[loop] La machine n'est pas en mode play, en attente du mode play..."
                    );
                ...
                // Comptage
                println!(
                    "[start_counting] Comptage en cours... Valeur actuelle : {}",
                    count
                );
                ...
                if is_debug {
                    ...
                    *state = StateMachine::Pause;
                    println!(
                        "[start_counting] État actuel après passage en pause : {:?}",
                        *state
                    );
                }
            ...
            *state = StateMachine::Idle;
            println!(
                "[start_counting] État actuel après retour à Idle : {:?}",
                *state
            );
            send_state_update(window, "idle");
        ...
        println!(
            "[start_counting] Fin de l'initialisation du comptage avec cible. Cible : {}",
            target
        );
    }
